function runExperiment(subjectId, partToRun, singleRun)
    ...
    runTiming = [blockWaitDuration:blockAndFixationDuration:blockAndFixationDuration * (numBlocks+1)]
    ...
    runTimingClass = [blockWaitDurationClass:blockAndFixationDurationClass:blockAndFixationDurationClass * (numBlocks)]

    numBlocksLocalizer = numBlocks;
    numImagesPerBlockLocalizer = numImagesPerBlock;
    runTimingLocalizer = [blockWaitDuration:blockAndFixationDuration:blockAndFixationDuration * (numBlocksLocalizer)]
    ...
    if runLocalizer
        ...
        try
            ...
            for i=1:numBlocksLocalizer
                ...
                blockStartTimes_localizer(1,end+1)= toc(startTic);
                ...
                for imgIdx = imageIndices
                    ...
                    imageStartTimes_localizer(1,end+1)= toc(startTic);
                    Screen('Flip', window);
                    ...
                    Screen('Flip', window);
                    imageEndTimes_localizer(1,end+1)= toc(startTic);

                    waitForTimeOrEsc(betweenShapesDuration);

                end % images in block
                    % log the end of this block:
                blockEndTimes_localizer(1,end+1)= toc(startTic);
        ...
        runEndTimes_localizer(1,end+1)= toc(startTic);
    ...
    if runFirstPart
        ...
        try
            ...
            for run = 1:numRuns
                ...
                runStartTimes(1,end+1)= toc(startTic);
                % start each run with a rest period
                Screen('DrawLines', window, fixationCoords, lineWidthFixation, black, [xCenter yCenter], 2);
                ...
                runEndTimes(1,end+1)= toc(startTic);
    ...
    if runSecondPart
        ...
        try
            ...
            for run = 1:numRunsClass
                ...
                % waitForMRI();
                startTic = tic;
                runStartTimesClass(1,end+1)= toc(startTic);
                ...
                for pair = 1:size(indexPairs,1)
                    ...
                    for i=1:numBlocksClass
                        ...
                        % log the start of this block
                        blockStartTimesClass(1, end+1) = toc(startTic);
                        for imgIdx = imageIndices % iterate over block images
                            ...
                            Screen('Flip', window);
                            imageStartTimesClass(1,end+1)= toc(startTic);
                            ...
                            Screen('DrawLines', window, fixationCoords, lineWidthFixation, black, [xCenter yCenter], 2);
                            Screen('Flip', window);
                            imageEndTimesClass(1,end+1)= toc(startTic);
                        ...
                        % log the end of this block
                        blockEndTimesClass(1, end+1) = toc(startTic);
                        Screen('DrawLines', window, fixationCoords, lineWidthFixation, black, [xCenter yCenter], 2);
                        Screen('Flip', window);
                        % waitForTimeOrEsc(blockWaitDurationClass);
                        waitForTimeOrEsc(runTimingClass(i + 1), 1, startTic); %force the next block to start on time
                ...
                runEndTimesClass(1,end+1)= toc(startTic);
